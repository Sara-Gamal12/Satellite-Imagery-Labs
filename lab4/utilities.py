...
def run(dataloaders,UNet,lr,step_size,num_epochs):
    num_class = 6

    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

    model = UNet(in_channels=3,out_channels=num_class).to(device)
    ...
    model,val_loss = train_model(dataloaders,model, optimizer_ft, exp_lr_scheduler, num_epochs=num_epochs)
...
def test(model,test_loader):
    if(test):
        ...
        print("Dice Score = ",dice_score(torch.from_numpy(pred),labels.cpu()).item())
        print("Jaccard Index = ",jaccard_index(torch.from_numpy(pred),labels.cpu()).item())
